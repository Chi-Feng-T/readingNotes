...
> * 系統需求分析亦不外如是，可以參考「Writing Effective Use Cases」一書，Usecase 也可被區分為不同的 goal level 層次，不像嗎？
...
程式碼重複，會讓你進行些微的流程修正時，需要改動多於一個的程式段落，而且容易有所遺漏，軟體發展史上的許多原則及慣例，主軸皆是消除或控制"重複"。如資料庫正規化、AOP、許多經典的 Design Patterns。

> It would appear that since the invention of the subroutine, innovations in software  developmenthave been an ongoing attempt to eliminate duplication from our source code.
...
> * 是否真要拆解到這麼細？ 它跟inline處理後的程式，哪一個使後續開發者較容易理解!? 就我看來見仁見智，又是一個在 ExtractMethod 與 InlineMethod 之間權衡的議題。
...
> * 再擴展到類別/模組/元件層面的話，要考量的事情又更多了。與 OCP 相關的經驗法則很重要、對 Domain/需求的分析理解很重要；實務上應消弭的重複是極常見的技術債務，專案管理上的權衡也很重要。
> * 不要為了把所有類似的流程塞在一起，而在主要流程的中間加入過多的判斷式，這可能會讓函式的邏輯分支變得過多而不易測試。
